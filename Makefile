...
# pipeline constants
# PROJECT_NAME
project_name := "clinic-2026-sample"
...
# Check that DATA_DIR reaches the Box folder (uses uv if installed, else Docker)
.PHONY: check-data
check-data:
	@if command -v uv >/dev/null; then \
		uv run python scripts/check_data.py --name $(USER); \
	else \
		docker compose run --rm $(project_name) python scripts/check_data.py --name $(USER); \
	fi
